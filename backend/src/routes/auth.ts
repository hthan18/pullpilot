  ...
  try {
    ...
    if (userResult.rows.length === 0) {
      ...
      const insertResult = await pool.query(
        'INSERT INTO users (github_id, username, email, avatar_url, access_token) VALUES ($1, $2, $3, $4, $5) RETURNING id',
        [
          githubUser.id.toString(),
          githubUser.login,
          githubUser.email,
      ...
      // Update existing user's access token + avatar
    ...
    // Store JWT in HTTP-only cookie (persistent login)
...
//Get current user info (reads from cookie via middleware)
